...
from unittest_reinvent.fixtures.test_data import METAMIZOLE_LABELED_COMPONENTS, METAMIZOLE_SCAFFOLD_FRAGMENT, \
    DIMETHYL_AMINO_PYRAZOLE, LINKER_THREE_SQUARE_BRACKETS, LINKER_CHARGED_ATTACHMENT_ATOM
...
class TestAttachmentPointModifier(unittest.TestCase):
    ...
    def test_extract_attachment_atoms(self):
        attachment_atoms = self.attachment_point_modifier.extract_attachment_atoms(self.linker_fragment_smiles)
        self.assertEqual(len(attachment_atoms), 2)
        self.assertEqual(attachment_atoms, ["CH2:1", "n:0"])

        selected_attachment_atoms = self.attachment_point_modifier.extract_attachment_atoms(self.linker_three_square_brackets)
    ...
    def test_add_explicit_H_to_atom(self):
        atom1 = self.attachment_point_modifier.extract_attachment_atoms(self.linker_fragment_smiles)[0]
        atom2 = self.attachment_point_modifier.extract_attachment_atoms(self.linker_fragment_smiles)[1]
        ...
        atom3 = self.attachment_point_modifier.extract_attachment_atoms(self.linker_three_square_brackets)[0]
        atom4 = self.attachment_point_modifier.extract_attachment_atoms(self.linker_three_square_brackets)[1]
        ...
        atom5 = self.attachment_point_modifier.extract_attachment_atoms(self.linker_charged_attachment_atom)[0]
        atom6 = self.attachment_point_modifier.extract_attachment_atoms(self.linker_charged_attachment_atom)[1]
